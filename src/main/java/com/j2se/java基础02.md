...
3. 通常情况下会在重写方法之上使用**@Override**注解，此方法可以检查重写方法的声明是否正确，以及表名此方法是重写方法
4. 方法重写的注意事项：
...
使用不同报下的类时，使用的时候要写类的全路径（及包含包名），为了简化带包的操作，java提供了导包的功能。
...
3. 状态修饰符
   ...
   - final（最终态）：final关键字时最终的意思，可以修饰类、成员变量、成员方法、局部变量
